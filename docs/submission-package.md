...
Our prototype turns that fragmented process into one guided mobile journey. The citizen photographs the ticket or enters its details manually. An OpenAI model extracts only the printed fields and marks anything unclear or missing. The citizen confirms those facts; fixed product rules—not AI—then check a simulated Railway cancellation record, ticket type and duplicate-claim ledger. Ownership is verified using a mock OTP sent to the booking mobile, followed by a synthetic UPI or bank payout and an auditable refund tracker.
...
- [ ] Public browser link that requests no access — **required blocker**
...
| Complete citizen journey | Capture → confirm → eligibility → ownership → payout → review → tracking | Meets |
| Simpler than the current experience | One guided flow with plain-language decisions and visible progress | Meets |
| Indian users and mobile | English/Hindi entry experience; 320–390 px layouts; large primary controls; manual fallback | Meets |
...
| Working, not static | Six interactive scenarios complete or terminate safely | Meets |
...
| Public submission URL | Localhost only; Sites is not enabled for this workspace | **Does not yet meet** |
