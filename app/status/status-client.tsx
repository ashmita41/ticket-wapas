    ...
    <main className="site-shell portal-shell">
      ...
      <div className="workspace portal-workspace">
        <section className="app-frame portal-frame" aria-live="polite">
          <div className="prototype-ribbon">{tr("Citizen account · Simulated sign-in · Synthetic refunds only", "नागरिक खाता · नकली साइन-इन · केवल नकली रिफंड")}</div>
          ...
          {stage === "signin" && <div className="screen portal-screen">
            ...
            <div className="bottom-actions"><button className="primary-button" disabled={!/^\d{10}$/.test(mobile)} onClick={() => go("otp")}>{tr("Send demo code", "डेमो कोड भेजें")} →</button></div>
          </div>}
          ...
          {stage === "otp" && <div className="screen portal-screen">
            ...
            <div className="screen-heading"><p className="eyebrow">{tr("SECURE SIGN-IN · SIMULATED", "सुरक्षित साइन-इन · नकली")}</p><h1>{tr("Enter the 6-digit code.", "6 अंकों का कोड भरें।")}</h1><p>{tr("Demo code sent to +91 •••••• 2714. No real SMS was sent.", "डेमो कोड +91 •••••• 2714 पर भेजा गया। कोई असली SMS नहीं भेजा गया।")}</p></div>
            <label className="portal-input-card otp-card"><span>{tr("ONE-TIME CODE", "एक बार का कोड")}</span><input className="auth-otp-input" aria-label={tr("Six-digit demo code", "छह अंकों का डेमो कोड")} inputMode="numeric" maxLength={6} placeholder="— — — — — —" value={otp} onChange={(event) => setOtp(event.target.value.replace(/\D/g, "").slice(0, 6))} /><small>{tr("This prototype accepts only the visible demo code.", "यह प्रोटोटाइप केवल दिखाया गया डेमो कोड स्वीकार करता है।")}</small></label>
          ...
          {stage === "account" && <div className="screen portal-screen account-screen">
            <div className="account-welcome"><div><p className="eyebrow">{tr("SIGNED IN · SYNTHETIC ACCOUNT", "साइन इन · नकली खाता")}</p><h1>{tr("Your refunds", "आपके रिफंड")}</h1><p>{tr("Signed in as Asha P. · +91 •••••• 2714", "आशा P. के रूप में साइन इन · +91 •••••• 2714")}</p></div><span className="account-avatar">AP</span></div>
            ...
            <button className="portal-text-signout" onClick={signOut}>{tr("Sign out of this demo account", "इस डेमो खाते से साइन आउट करें")}</button>
          </div>}
          ...
          {stage === "detail" && <div className="screen portal-screen claim-detail-screen">
            ...
            <div className="help-card"><b>{tr("Need help with this refund?", "इस रिफंड में सहायता चाहिए?")}</b><p>{tr(`Quote ${selectedClaim.id} when contacting the authorised support channel. This prototype does not contact a real support team.`, `अधिकृत सहायता चैनल से संपर्क करते समय ${selectedClaim.id} बताएँ। यह प्रोटोटाइप किसी असली सहायता टीम से संपर्क नहीं करता।`)}</p></div>
          </div>}
        </section>
      </div>

      <footer className="site-footer portal-footer"><b>Ticket Wapas · टिकट वापस</b><span>{tr("Independent civic-tech prototype. Not affiliated with Indian Railways, IRCTC or the Government of India.", "स्वतंत्र नागरिक-तकनीक प्रोटोटाइप। भारतीय रेल, IRCTC या भारत सरकार से संबद्ध नहीं।")}</span><span><Link href="/authority">{tr("Authority operations demo", "प्राधिकरण संचालन डेमो")}</Link></span></footer>
    </main>
  );
}
